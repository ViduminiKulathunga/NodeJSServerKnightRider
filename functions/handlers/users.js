...
exports.registerUser = (req, res) => {
  ...
  const newRFID = {
    ...
    drunken: false,
    handle: req.body.handle,
    location: locationInit, //new admin.firestore.GeoPoint(0, 0),
    tripId: "",
    login: false,
    hasPassenger: false,
        ...
        firebase
          ...
          .then((idToken) => {
            ...
            if (newUser.role == "driver") {
              ...
              newRFID.rfid = newUser.rfid;
              //create RFID account
              db.doc(`driverlog/${newUser.rfid}`)
                .set(newRFID)
                .then(() => {
          ...
          .catch((err) => {
            ...
            if (err.code === "auth/email-already-in-use") {
              return res
                .status(400)
                .json({ email: "Email is already is in use" });
            } else {
              //console.log(err);
              return res
                .status(500)
...
exports.setUserLog = (req, res) => {
  ...
  if (req.body.role == "driver") {
    ...
    setLog.drunkenPesentage = req.body.drunkenPesentage;
  }
...
exports.calculateDriverSalary = (req, res) => {
  const user = {
    inputYear: req.params.year, //web fixed month and year right now
    inputMonth: req.params.month,
  };
  ...
  db.collection("drivercommission")
    ...
    .then(() => {
      db.collection("alcohol")
        ...
        .get()
        .then((data) => {
          driverData.alcoholStatus = [];
          data.forEach((doc) => {
            driverData.alcoholStatus.push(doc.data());
          });
          return res.json(driverData);
        });
    })
